...
def main():
	d=detect(inputFolder='../data/poe-4/',outputFolder='../output/poe-4-2/')
	...
	reducedBooks=d.filterWithJacard(textChunks,books,threshold=0.04)
	pickling_on = open('../output/'+'poe-4-2/reducedBooks.pickle',"wb")
	...
	pickling_on = open('../output/'+'poe-4-2/parseTrees.pickle',"wb")
	...
	pickling_on = open('../output/'+'poe-4-2/potentialParseTrees.pickle',"wb")
	...
	pickling_on = open('../output/'+'poe-4-2/allScores.pickle',"wb")
	...
	pickling_on = open('../output/'+'poe-4-2/scoreTuples.pickle',"wb")
	pickle.dump(scoreTuples, pickling_on)

	finalTuples,diffTuples=d.finalFiltering(scoreTuples,reducedBooks,0.70)
	if len(finalTuples)>100:
		finalTuples=finalTuples[0:100]
	orderedTuples=d.nounBasedRanking(finalTuples,text,reducedBooks)
	
	pickling_on = open('../output/'+'poe-4-2/orderedTuples.pickle',"wb")
	...
	pickling_on = open('../output/'+'poe-4-2/diffTuples.pickle',"wb")
